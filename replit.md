...
- **Backend**: extends `artifacts/api-server` (Express 5 on `/api`).
  ...
  - `GET /api/tts/health`, `POST /api/tts/speech` — OpenAI TTS proxy with configurable backend cache. Portable environments default to filesystem cache (`TTS_CACHE_PROVIDER=filesystem`); Replit can opt into Object Storage with `TTS_CACHE_PROVIDER=replit-object-storage`. Cache keys use `sha256(text|model|voice|speed)` for collision resistance.
