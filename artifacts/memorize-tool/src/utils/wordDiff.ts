...
export function tokenizeComparableText(text: string): string[] {
  ...
  return comparableText.split(/\s+/).filter((w) => w.length > 0);
}
...
export function computeWordDiff(expected: string, spoken: string): WordDiff[] {
  ...
  if (expectedWords.length === 0 && spokenWords.length === 0) return [];
  if (spokenWords.length === 0) return expectedWords.map((word) => ({ word, status: "missing" }));
  if (expectedWords.length === 0) return spokenWords.map((word) => ({ word, status: "extra" }));
...
export function hasErrors(diff: WordDiff[]): boolean {
  return diff.some((item) => item.status !== "correct");
}
